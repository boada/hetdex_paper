...
The halo catalogs are created using a friends-of-friends algorithm \citeeg{Turner1976} with linking length, $b=0.2$. The halos are populated with galaxies using a semi-analytical model described in \cite{Croton2006, DeLucia2006, DeLucia2007a}. 
...
Cluster masses are assigned using the ML methods (see Section \ref{sec:machine learning method}) using two different training sets. We use the full set of clusters detected in the Buzzard catalogs (14000 clusters with $M > 10^{11}$ \msol) and a training-testing split of the Millennium clusters. 

\begin{figure} 
	\includegraphics[width=\columnwidth]{figures/millBuzzComparison.pdf} 
	\caption{Mass predictions for the power law scaling relation (Equation~\ref{eq:power law}) and the ML based technique with different input features as a function of true cluster mass. The bottom row of panels shows the fractional error (Equation~\ref{eq: fractional error}) also as a function of true cluster mass. The solid black line shows the 1:1 relation. The solid, colored line is the median predicted mass for the targeted observing, and the colored, dashed line is the median recovered mass for the HETDEX-like observations. The shaded regions represent the 68\% scatter around the median values.} \label{fig: mill buzz comparison} 
\end{figure}


The predicted cluster masses for the 4000 millennium cluster are very similar regardless of the training set used. I've attached a plot that shows the probability of the predicted masses using the two different training sets. I also computed the mean squared error (MSE) of each set, compared to the *true* cluster mass. I like the MSE because it takes into account both the scatter and the bias. For the millennium clusters I found MSE $= 0.015$, and for the Buzzard clusters MSE $= 0.029$. Smaller numbers are better, and I think the difference is because I have chosen not to exclude clusters with $Msol < 1e13$ from the Buzzard catalogs when training. That allows the ML method to assign clusters below that which ups the MSE.

I'm hoping that Millennium is different enough from Buzzard that this will address your concern. Another option would be just to include training samples from a few different mocks. Or, we could derive masses using different training sets and then marginalize over them to get a final predicted mass. 
...
To get a sense of how well HETDEX will be able to constrain cosmological parameters we follow the discussion of \cite{Weinberg2013}, and begin with a few simplifying assumptions. While sensitive to $\Omega_m$, the number density of clusters do not necessarily provide the strongest contraint, so we assume that some combination of other data sets (\eg, CMB, BAO, supernovae, WL, etc.) will constrain $\Omega_m$ to high precision. 
...
\begin{equation}
	\Delta~\mathrm{ln}~\sigma_{11,abs}(z) = q \times \mathrm{max}[\Delta~\mathrm{ln}~M,\alpha^{-1}\Delta\mathrm{ln}~N]
\end{equation}
where $q$ is the degeneracy exponent between $\sigma_8h^{-1}$ and $\Omega_m$, $\Delta\mathrm{ln}M$ is the mass scale uncertainty, $\Delta\mathrm{ln}N$ is the cluster statistical uncertainty, and $\alpha$ is slope of the cumulative HMF. Using the \cite{Tinker2008} HMF \cite{Weinberg2013} estimate $q\sim0.4$, and find that for any cluster survey with more than 10-20 clusters is dominated by the uncertainty in the overall mass scale.

For a survey such as HETDEX, with a power law derived overall cluster mass biases of $>0.15$ dex (about 41\%) the uncertainty on $\sigma_8h^{-1}$ is $0.4 \times 0.41 \approx 16\%$. If we consider only clusters with masses above $10^{14}$ \Msol, with perfect knowledge, and the ML based cluster mass estimates the mass scale uncertainty is reduced to $\sim -0.02$ dex or about 5\%. This gives a uncertainty on $\sigma_8h^{-1}$ of 2\%. For clusters above $10^{14}$ \Msol, survey observations can constrain the masses to about 20\% which, in turn, constrains $\sigma_8h^{-1}$ to 8\%. 

Because of the simplifying assumptions, and the superior quality of the data (no contamination, signal-to-noise issues, etc.), realistic expectations for HETDEX is to directly constrain $\sigma_8h^{-1}$ to better precision than other methods (\eg, CMB, WL, X-ray) seems bleak. For the $\sim1500$ clusters detected with survey observations to constrain $\sigma_8h^{-1}$ and to be dominated by cluster statistics alone ($\Delta\mathrm{ln}N \sim N^{-1/2}$), the absolute cluster calibration would need to be better than 2.5\%. For a fully targeted survey, about 14000 clusters, this cluster mass calibration uncertainty reduces to $>1\%$.    
