...
\section{ {\rm[\ion{O}{ii}]} Luminosity}
HETDEX is designed to detect Lyman-$\alpha$ at redshift two. That allows us to detect [\ion{O}{ii}] ($\lambda\lambda 3727-3729\AAA$ doublet) emitters to $z\sim 0.49$. The catalog does not provide [\ion{O}{ii}] luminosities so we must assign them empirically. We use 503113 galaxies from the Sloan Digital Sky Survey (SDSS) Data Release 12 \citep{Alam2015} from $z = 0.05 - 0.2$, which are selected which have no redshift warning. 
