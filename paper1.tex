...
Figure~\ref{fig: oii sdss} illustrates this process. The numbered boxes in the left panel show the bins corresponding to two example Buzzard galaxies ($M_r, g-r = -17.7,~0.49$ and $M_r, g-r = -21.4,~1.24$). The right panel shows the Log [\ion{O}{ii}] luminosity distribution functions, $p($[\ion{O}{ii}]$| M_r, g-r)$, which we use to assign [\ion{O}{ii}] luminosity to each object. The luminosity is then converted into an [\ion{O}{ii}] flux through 
\begin{equation}
	F = \frac{L}{4\pi D_L}
\end{equation}
