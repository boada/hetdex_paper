...
We repeat our analysis using cluster halo and \cite{DeLucia2007} galaxy catalogs derived from the Millennium simulation and obtained via querying the Millennium online database\footnote{\url{http://gavo.mpa-garching.mpg.de/Millennium/}} \citep{Lemson2006}. The Millennium simulation tracks $2160^3$ dark matter particles of $8.6\times 10^8 ~h^{-1}$ \Msol\ inside a comoving 500 $(h^{-1} \mathrm{Mpc})^3$ box from $z=127$ to 0. The following SQL query selects the cluster halos:
\begin{lstlisting}[language=SQL]
SELECT *
FROM   mfield..fof
